...
Real pres_init(const Real bx, const Real by, const Real bz, const int num_cs,
    ...
    const Real phi_pert, const Real beta0, const Real lx, const Real xleft, 
    const Real xright, const Real xcenter);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real phi_pert, vin_pert, vin_shear;
  ...
  int pert_shear = 0;   // In default, there is no velocity shear
  ...
  vin_shear = pin->GetReal("problem", "vin_shear");
  ...
  pert_shear = pin->GetInteger("problem", "pert_shear");
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
        ...
        pgas_nr = pres_init(pfield->bcc(IB1,k,j,i), pfield->bcc(IB2,k,j,i),
                ...
                phi_pert, beta0, lx, xleft, xright, xcenter);
        // density
        if (uniform_rho)
