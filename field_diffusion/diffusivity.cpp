...
//! \brief EMF from Hall resistivity

void FieldDiffusion::HallEMF(const FaceField &b, const AthenaArray<Real> &bc,
                              EdgeField &e) {
                                  
  // This first part is the same for all types of diffusion
  MeshBlock *pmb = pmy_block;
  int is = pmb->is; int js = pmb->js; int ks = pmb->ks;
  int ie = pmb->ie; int je = pmb->je; int ke = pmb->ke;
  ...
  if (pmb->block_size.nx2 == 1) {
    for (int i=is; i<=ie+1; ++i) {
      ...
      Real JcrossB2 = J3(ks,js,i)*intBx - J1(ks,js,i)*intBz;
      Real JcrossB3 = J1(ks,js,i)*intBy - J2(ks,js,i)*intBx;

      e2(ks  ,js  ,i) += eta_H * JcrossB2;
      e2(ke+1,js  ,i)  = e2(ks,js,i);
      e3(ks  ,js  ,i) += eta_H * JcrossB3;
      e3(ks  ,je+1,i)  = e3(ks,js,i);
    }
    return;
  }
...
#pragma omp simd
      for (int i=is; i<=ie+1; ++i) {
        ...
        Real JcrossB1 = intJy*intBz - intJz*intBy;
        Real JcrossB2 = intJz*intBx - intJx*intBz;
        Real JcrossB3 = intJx*intBy - intJy*intBx;
        
        e1(ks  ,j,i) += eta_H * JcrossB1;
        e1(ke+1,j,i)  = e1(ks,j,i);
        ...
        JcrossB1 = intJy*intBz - intJz*intBy;
        JcrossB2 = intJz*intBx - intJx*intBz;
        JcrossB3 = intJx*intBy - intJy*intBx;
        
        e2(ks  ,j,i) += eta_H * JcrossB2;
        e2(ke+1,j,i)  = e2(ks,j,i);
