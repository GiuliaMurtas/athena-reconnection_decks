...
// Diffisivity (initialized to the initial condition file value
void VariDiffusivity(FieldDiffusion *pfdif, MeshBlock *pmb,
    ...
    #pragma omp simd
                for (int i = is; i <= ie; i++) {
                    // 0. initialize eta according to the input
                    Real x2c = 0.5 * (pmb->pcoord->x2f(j) + pmb->pcoord->x2f(j + 1));
                    Real x1c = 0.5 * (pmb->pcoord->x1f(j) + pmb->pcoord->x1f(j + 1));
                    Real w_eta = 0.001736;
                    Real wq2 = 2.0*SQR(w_eta);
                    Real ycenter = 2.0;
                    Real xcenter = 0.0;
                    pfdif->etaB(0, k, j, i) = pfdif->eta_ohm * exp(-(SQR(x1c - xcenter)+SQR(x2c - ycenter))/wq2) + pfdif->eta_ohm * exp(-(SQR(x1c + xcenter)+SQR(x2c - ycenter))/wq2);
                }
            }
        }
    }
...
// Magnetic field perturbation (Bx)
//static Real func_bxini_1(const Real x1, const Real x2, const int num_cs)
//{
    /* Add perturbation */
//    Real pix, piy, bx1;
//    Real xc_symm =0.0;
...
//    Real ly_psipert = 1.0;
//    Real yc_psipert = 0.0;
//    Real psi_pert = 0.00001;
//    Real pi=3.141592653589793;
//    Real b0 = 1.0;
    
//    pix = pi*(x1-x1_symm)/lx_psipert;  
...
//    return bx1;
    ...
    //  return bx1;
    //} else if (num_cs == 2) {
    //  pix = pi*(x1-x1_symm)/lx_psipert + pi*(x1-x2_symm)/lx_psipert;  
    //  piy = 2.0*pi*(x2-yc_psipert)/ly_psipert;
    //  bx1 = (2.0*pi/ly_psipert)*psi_pert*cos(pix)*sin(piy); 
      //pix = pi*(x1-x2_symm)/lx_psipert;
      //piy = 2.0*pi*(x2-yc_psipert)/ly_psipert;
      //bx1 = (2.0*pi/ly_psipert)*psi_pert*cos(pix)*sin(piy);
    //  return bx1;
    //} else {
...
// Magnetic field perturbation (By)
//static Real func_byini_1(const Real x1, const Real x2, const int num_cs)
//{
//    Real pix, piy, by1;
//    Real xc_symm =0.0;
//    Real x1_symm =-0.5;
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // Real w_eta, psi_pert, lx_psipert, ly_psipert, yc_psipert;
    
    // Setup of Gaussian function for resistivity
#ifdef RESISTIVITY
    void get_eta_user(GridS *pGrid, int i, int j, int k,
    Real *eta_O, Real *eta_H, Real *eta_A)
    {
        ...
        *eta_A = 0.0;
        ...
        //xcenter = -0.5; // for 2 current sheets
        //ycenter = 2.0; // for 2 current sheets
        ycenter = 2.0; // for 1 current sheet
        xcenter = 0.0; // for 1 current sheet
        Real factor = exp(-(SQR(x1c - xcenter)+SQR(x2c - ycenter))/wq2) + exp(-(SQR(x1c + xcenter)+SQR(x2c - ycenter))/wq2);
            *eta_O = eta_ohm_origin*factor;
    }
    return;
    }
#endif
  ...
  for (int j=js; j<=je; j++) {
    for (int i=is; i<=ie; i++) {
      ...
      if (uniform_rho)
        ...
        phydro->u(IDN,k,j,i) = 1.0+(100000 -1)*(1-tanh((pcoord->x2v(j)-0.05)/0.2))/2.0;
      // momentum
      if (pert_V) {
        if (random_vpert) {
